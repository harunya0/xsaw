...
| | 拡張子フィルタ (`-e`, 複数指定対応) | ◯ | ◯ | **完了** |
| | 標準入力パイプ集約 (`du -`, `\| du`) | ◯ | ◯ | **完了** |
| **File Search** | 仮想スレッド並行検索 (`fi`, `f`) | ◯ | ◯ | **完了** (47 tests PASS) |
...
### ✅ 乖離 1: ファイル移動コマンド (`xsaw mv`, `m`) 【解消済み】
- **README の記述**:
  `xsaw mv <source> <destination>` でファイルを移動する。
- **現在のコードベース**:
  - `App.java` に `MvCommand`（エイリアス `m`）を実装。
  - `mv.java` に移動前存在チェック、既存ディレクトリ内への自動展開、親ディレクトリ自動生成、クロスドライブ対応を実装。
  - 単体テスト・CLIテスト完備。
- **完了タスク**:
  - [x] `MvCommand` の picocli アノテーション定義（エイリアス `m` 対応）。
  - [x] 移動元ファイル・移動先パスの存在チェック・バリデーション。
  - [x] `Files.move` によるアトミック/安全なファイル移動処理。
